...
export async function jacobMethodPrecision(_A: number[][], _B: number[], precision: number = 0.001) {
...
export default async function jacobMethodIterations(_A: number[][], _B: number[], Iterations: number) {
    const A = JSON.parse(JSON.stringify(_A));
    const B = JSON.parse(JSON.stringify(_B));
    let x: number[] = Array(A.length).fill(0);
    let allResults: number[][] = [];
    for (let _ = 0; _ < Iterations; _++) {
        // const previusX = x.slice();
        for (let i = 0; i < A.length; i++) {
